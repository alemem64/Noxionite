...
class GraphControlAPI {
  ...
  private getTagsBySlug(slug: string): string[] {
    ...
    const blockWrapper =
      this.recordMap?.block?.[pageInfo.pageId] ||
      this.recordMap?.[pageInfo.pageId]
    ...
    if (block?.properties) {
      ...
      if (tagsProperty) {
        ...
        if (Array.isArray(tagsProperty)) {
          ...
          typeof tagsProperty === 'string' &&
          tagsProperty.trim().length > 0
        ) {
          ...
          if (tagsProperty.multi_select) {
            ...
            tagsProperty.name &&
            typeof tagsProperty.name === 'string' &&
            tagsProperty.name.trim().length > 0
          ) {
    ...
    if (Array.isArray(rawTags)) {
      tags = rawTags
        .filter(
          (tag: unknown) =>
            typeof tag === 'string' && (tag as string).trim().length > 0
        )
        .map((tag: unknown) => (tag as string).trim())
    } else if (
      typeof rawTags === 'string' &&
      (rawTags as string).trim().length > 0
    ) {
      ...
      if (tagObj.multi_select) {
        ...
        (tagObj.name as string).trim().length > 0
      ) {
  ...
  addListener(
    instanceType: string,
    callback: (message: GraphControlMessage) => void
  ) {
   ...
   */
  handleUrlInitialFocus(
    pathname: string,
    instanceType: 'sidenav' | 'home' = 'sidenav'
  ) {
   ...
   */
  private setInitialFocusRequest(request: {
    segment: string
    slug: string
    instanceType: 'sidenav' | 'home'
  }) {
  ...
  handleUrlCurrentFocus(
    ...
    const effectiveCurrentView =
      currentView ||
      this.instanceStates.get(instanceType)?.currentView ||
      'post_view'
        ...
        if (slugs.length === 1) {
          ...
          this.sendMessage({
            ...
            payload: {
              slug: slugs[0],
              options,
              continuous: continuous ?? needsViewChange
            }
          })
        } else {
          ...
          this.sendMessage({
            ...
            payload: {
              slugs,
              options: { ...options, continuous: continuous ?? needsViewChange }
            }
          })
        }
      },
      ...
      () => {
        if (nodeIds.length === 1) {
          ...
          this.sendMessage({
            ...
            payload: {
              nodeIds,
              options: { ...options, continuous: continuous ?? true }
            }
          })
        }
      },
...
export function calculateZoomLevel(
  ...
  const targetCanvasHeight =
    canvasHeight * GRAPH_CONFIG.zoom.MULTIPLE_ZOOM_RATIO
